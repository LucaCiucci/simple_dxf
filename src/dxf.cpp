...
namespace simple_dxf
{
	...
	void CDxfWrite::writeHeaderSection(void)
	{
		std::stringstream ss;
		ss << "FreeCAD v"
			<< 0// ! App::Application::Config()["BuildVersionMajor"]
			<< "."
			<< 0// ! App::Application::Config()["BuildVersionMinor"]
			<< " "
			<< 0;// ! App::Application::Config()["BuildRevision"];
		...
		ss.str("");
		ss.clear();
		ss << "DxfPlate/header" << m_version << ".rub";
	...
	void CDxfWrite::writeTablesSection(void)
	{
		...
		std::stringstream ss;
		ss << "DxfPlate/tables1" << m_version << ".rub";
		std::string fileSpec = m_dataDir + ss.str();
		(*m_ofs) << getPlateFile(fileSpec);
		...
		ss.str("");
		ss.clear();
		ss << "DxfPlate/tables2" << m_version << ".rub";
	...
	std::string CDxfWrite::getPlateFile(std::string fileSpec)
	{
		...
		const bool readable = [&fileSpec]() {
			std::ifstream f(fileSpec);
			return f.good();
		}();
		if (!readable) {
		//    Base::Console().Message("dxf unable to open %s!\n", fileSpec.c_str());
			std::cout << "dxf unable to open " << fileSpec << "!" << std::endl;
		}
		else {
			std::string line;
			std::ifstream inFile(fileSpec.c_str());
	...
	void CDxfWrite::writeLine(const Vector3d& s, const Vector3d& e)
	{
		putLine(Vector3d(s), Vector3d(e), m_ssEntity, getEntityHandle(), m_saveModelSpaceHandle);
	}

	void CDxfWrite::putLine(const Vector3d& s, const Vector3d& e,
	...
	void CDxfWrite::writePoint(const Vector3d& s)
	{
		(*m_ssEntity) << "  0" << std::endl;
	...
	void CDxfWrite::writeArc(const Vector3d& s, const Vector3d& e, const Vector3d& c, bool dir)
	{
		double ax = s[0] - c[0];
		double ay = s[1] - c[1];
		double bx = e[0] - c[0];
	...
	void CDxfWrite::writeCircle(const Vector3d& c, double radius)
	{
		(*m_ssEntity) << "  0" << std::endl;
	...
	void CDxfWrite::writeEllipse(const Vector3d& c, double major_radius, double minor_radius,
	...
	void CDxfWrite::writeText(const char* text, const Vector3d& location1, const Vector3d& location2,
		const double height, const int horizJust)
	{
		putText(text, Vector3d(location1), Vector3d(location2),
			height, horizJust,
	...
	//putText
	//added by Wandererfan 2018 (wandererfan@gmail.com) for FreeCAD project
	void CDxfWrite::putText(const char* text, const Vector3d& location1, const Vector3d& location2,
		const double height, const int horizJust,
		std::ostringstream* outStream, const std::string handle,
		const std::string ownerHandle)
	{
	...
	void CDxfWrite::putArrow(const Vector3d& arrowPos, const Vector3d& barb1Pos, const Vector3d& barb2Pos,
		std::ostringstream* outStream, const std::string handle,
		const std::string ownerHandle)
	{
		(*outStream) << "  0" << std::endl;
	...
	#define HORIZONTAL 1
	#define VERTICAL 2
	void CDxfWrite::writeLinearDim(const Vector3d& textMidPoint, const Vector3d& lineDefPoint,
	...
	//writeAngularDim
	//added by Wandererfan 2018 (wandererfan@gmail.com) for FreeCAD project
	void CDxfWrite::writeAngularDim(const Vector3d& textMidPoint, const Vector3d& lineDefPoint,
	...
	//writeRadialDim
	//added by Wandererfan 2018 (wandererfan@gmail.com) for FreeCAD project
	void CDxfWrite::writeRadialDim(const Vector3d& centerPoint, const Vector3d& textMidPoint,
	...
	//writeDiametricDim
	//added by Wandererfan 2018 (wandererfan@gmail.com) for FreeCAD project
	void CDxfWrite::writeDiametricDim(const Vector3d& textMidPoint,
	...
	//writeLinearDimBlock
	//added by Wandererfan 2018 (wandererfan@gmail.com) for FreeCAD project
	void CDxfWrite::writeLinearDimBlock(const Vector3d& textMidPoint, const Vector3d& dimLine,
		const Vector3d& e1Start, const Vector3d& e2Start,
		const char* dimText, int type)
	{
		Vector3d e1S(e1Start[0], e1Start[1], e1Start[2]);
		Vector3d e2S(e2Start[0], e2Start[1], e2Start[2]);
		Vector3d dl(dimLine[0], dimLine[1], dimLine[2]);      //point on DimLine (somewhere!)
		Vector3d perp = dl.distanceToLineSegment(e2S, e1S);
		...
		putText(dimText, Vector3d(textMidPoint), Vector3d(dimLine), 3.5, 1,
	...
	//added by Wandererfan 2018 (wandererfan@gmail.com) for FreeCAD project
	void CDxfWrite::writeAngularDimBlock(const Vector3d& textMidPoint, const Vector3d& lineDefPoint,
		const Vector3d& startExt1, const Vector3d& endExt1,
		const Vector3d& startExt2, const Vector3d& endExt2,
		const char* dimText)
	{
		...
		double radius = length(e2S - linePt);
		...
		putText(dimText, Vector3d(textMidPoint), Vector3d(textMidPoint), 3.5, 1,
			m_ssBlock, getBlockHandle(), m_saveBlkRecordHandle);
	...
	//writeRadialDimBlock
	//added by Wandererfan 2018 (wandererfan@gmail.com) for FreeCAD project
	void CDxfWrite::writeRadialDimBlock(const Vector3d& centerPoint, const Vector3d& textMidPoint,
		const Vector3d& arcPoint, const char* dimText)
	{
		putLine(Vector3d(centerPoint), Vector3d(arcPoint),
			m_ssBlock, getBlockHandle(),
			m_saveBlkRecordHandle);
	...
	//added by Wandererfan 2018 (wandererfan@gmail.com) for FreeCAD project
	void CDxfWrite::writeDiametricDimBlock(const Vector3d& textMidPoint,
		const Vector3d& arcPoint1, const Vector3d& arcPoint2,
		const char* dimText)
	{
		putLine(Vector3d(arcPoint1), Vector3d(arcPoint2),
			m_ssBlock, getBlockHandle(),
			m_saveBlkRecordHandle);
	...
	void CDxfWrite::writeBlocksSection(void)
	{
		if (m_version < 14) {
			std::stringstream ss;
			ss << "DxfPlate/blocks1" << m_version << ".rub";
			std::string fileSpec = m_dataDir + ss.str();
			(*m_ofs) << getPlateFile(fileSpec);
		}
	...
	{
		std::stringstream ss;
		ss << "DxfPlate/entities" << m_version << ".rub";
		std::string fileSpec = m_dataDir + ss.str();
		(*m_ofs) << getPlateFile(fileSpec);
	...
	{
		Vector3d c = { 0,0,0 }; // centre
		Vector3d m = { 0,0,0 }; //major axis point
		...
		try {
			if (poly_prev_found)
			{
				...
				if (poly_prev_bulge_found)
				{
					...
					Vector3d ps = { poly_prev_x, poly_prev_y, poly_prev_z };
					Vector3d pe = { x, y, z };
					Vector3d pc = { cx, cy, (poly_prev_z + z) / 2.0 };
				...
				if (!arc_done)
				{
					Vector3d s = { poly_prev_x, poly_prev_y, poly_prev_z };
					Vector3d e = { x, y, z };
	...
	void CDxfRead::OnReadArc(double start_angle, double end_angle, double radius, const Vector3d& c, double z_extrusion_dir, bool hidden) {
		Vector3d s = { 0,0,0 }, e = { 0,0,0 }, temp = { 0,0,0 };
	...
	void CDxfRead::OnReadCircle(const Vector3d& c, double radius, bool hidden) {
		Vector3d s;
	...
	void CDxfRead::OnReadEllipse(const Vector3d& c, const Vector3d& m, double ratio, double start_angle, double end_angle) {
	...
	{
		Vector3d c = { 0,0,0 }; // coordinate
		Vector3d s = { 1,1,1 }; // scale
	...
	{
		Vector3d s = { 0,0,0 }; // startpoint
		Vector3d e = { 0,0,0 }; // endpoint
		Vector3d p = { 0,0,0 }; // dimpoint
